...
const accessTokenCookieOptions: CookieOptions = {
  ...cookiesOptions,
  expires: new Date(
    Date.now() + config.get<number>('access_tokenExpiresIn') * 60 * 1000
  ),
  maxAge: config.get<number>('access_tokenExpiresIn') * 60 * 1000,
};
...
export const refreshaccess_tokenHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
